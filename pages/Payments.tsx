...
export const Payments: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
        const [payData, bookData] = await Promise.all([
            api.get<Payment[]>('http://192.168.43.171:5000/api/payments'),
  ...
  const handleSubmit = async () => {
     ...
     try {
        if(editingPayment && editingPayment.payment_id) {
            await api.put(`http://192.168.43.171:5000/api/payments/${editingPayment.payment_id}`, payload);
            showNotification('Payment updated', 'success');
        } else {
            await api.post('http://192.168.43.171:5000/api/payments', payload);
  ...
  const handleDelete = async () => {
      ...
      try {
          await api.delete(`http://192.168.43.171:5000/api/payments/${deletingPayment.payment_id}`);
