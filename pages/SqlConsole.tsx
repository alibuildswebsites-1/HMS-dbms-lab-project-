  ...
  return (
    <Layout title="SQL Console">
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Header Section */}
        <div className="bg-gradient-to-r from-[#2C4A3B] to-[#4A7C59] p-6 rounded-2xl text-white shadow-lg relative overflow-hidden">
          <div className="absolute right-0 top-0 opacity-10 transform translate-x-10 -translate-y-10">
            <Database size={150} />
          </div>
          <div className="relative z-10">
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <Terminal size={24} className="text-[#F4D35E]" />
              Query Executor
            </h2>
            <p className="text-gray-200 mt-1 opacity-90">Execute raw SQL commands directly against the database.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
              <div className="bg-[#2C4A3B] px-4 py-2 flex justify-between items-center border-b border-[#4A7C59]">
                <span className="text-xs font-mono text-[#F4D35E] uppercase tracking-wider">SQL Editor</span>
                <div className="flex gap-2">
                   <button 
                    onClick={handleClear}
                    className="text-gray-300 hover:text-white transition-colors text-xs flex items-center gap-1 px-2 py-1 rounded hover:bg-white/10"
                   >
              ...
              <textarea
                ...
                className="w-full h-64 p-4 bg-[#1e293b] text-green-400 font-mono text-sm focus:outline-none resize-y"
                spellCheck={false}
              />
              <div className="bg-gray-50 p-3 border-t border-gray-200 flex justify-end gap-3">
                 <button
                  ...
                  disabled={isLoading}
                  className="bg-[#4A7C59] text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-[#3B6347] transition-all shadow-lg shadow-[#4A7C59]/20 disabled:opacity-70 disabled:cursor-not-allowed hover:scale-105 active:scale-95"
                 >
                   {isLoading ? (
              ...
              <div className="animate-in fade-in zoom-in duration-300">
                {error ? (
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="bg-[#E6F4EA] px-6 py-3 border-b border-[#4A7C59]/20 flex justify-between items-center text-[#2C4A3B]">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
              <h3 className="text-[#2C4A3B] font-bold text-sm uppercase tracking-wider mb-4 border-b border-gray-100 pb-2">Example Queries</h3>
              <div className="flex flex-col gap-2">
                {examples.map((ex, idx) => (
                  <button
                    key={idx}
                    onClick={() => setQuery(ex)}
                    className="text-left text-xs p-3 rounded-xl bg-gray-50 hover:bg-[#E6F4EA] hover:text-[#2C4A3B] hover:border-[#4A7C59]/30 border border-transparent transition-all duration-200 group truncate"
                    title={ex}
                  >
                    <span className="font-mono text-gray-500 group-hover:text-[#4A7C59] block truncate">
                      {ex}
                    </span>
                  </button>
                ))}
            ...
            {/* History */}
            {history.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 animate-in fade-in slide-in-from-right-4">
                <h3 className="text-[#2C4A3B] font-bold text-sm uppercase tracking-wider mb-4 border-b border-gray-100 pb-2 flex justify-between items-center">
                  Recent History
                ...
                <div className="flex flex-col gap-2">
                  {history.map((h, idx) => (
                    <button
                      key={idx}
                      onClick={() => setQuery(h)}
                      className="text-left text-xs p-3 rounded-xl border border-gray-100 hover:border-[#F4D35E] hover:bg-[#FFF8E1] transition-all duration-200 truncate font-mono text-gray-600"
                      title={h}
                    >
                      {h}
                    </button>
